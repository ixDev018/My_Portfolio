                        ...
                        <style>.ui-track::-webkit-scrollbar { display: none; }</style>
                            ...
                            @forelse($uiProjects as $index => $proj)
                                ...
                                <a href="{{ $cardHrefUi }}"
                                   ...
                                   @click="if(isMobile && mobileFocusId !== itemId && $el.getAttribute('href') === '#') { $event.preventDefault(); mobileFocusId = itemId; dimming = true; window.dispatchEvent(new CustomEvent('mobile-focus-reset', { detail: { id: itemId } })); } @if($onClickUi) else { {!! $onClickUi !!} } @endif"
