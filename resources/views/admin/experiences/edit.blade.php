...
<form action="{{ route('admin.experiences.update', $experience->id) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="pe-shell">
        ...
        <div class="pe-editor-col">
            ...
            <div class="pe-editor-header">
                ...
                <input type="text" name="duration" class="pe-subtitle-input"
                       ...
                       value="{{ old('duration', $experience->duration) }}" required>
        ...
        {{-- ═══ RIGHT: Sidebar ═══ --}}
        <div class="pe-sidebar" x-data="{ tab: 'media', is_active: {{ old('is_active', $experience->is_active ?? false) ? 'true' : 'false' }}, bg_media_type: '{{ old('bg_media_type', $experience->bg_media_type ?? 'image') }}' }">
            ...
            <div class="pe-sidebar-scroll">
                ...
                <div x-show="tab === 'media'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" style="display:none;">
                    ...
                    <div style="margin-bottom:0.5rem; border-radius:0.5rem; overflow:hidden; border:1px solid #D8D4C8;">
                        <img src="{{ asset('storage/' . $experience->image_path) }}" alt="Current Image" style="width:100%; height:auto; display:block;">
                    </div>
                    @endif
                ...
                <div x-show="tab === 'bg'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" style="display:none;">
                    ...
                    <div class="pe-field" x-show="bg_media_type === 'image' || bg_media_type === 'video'" style="margin-top:0.75rem;">
                        ...
                        @if($experience->bg_media_path)
                            <div style="margin-bottom:0.5rem; border-radius:0.5rem; overflow:hidden; border:1px solid #D8D4C8;">
                                @if($experience->bg_media_type == 'video')
                                    <video src="{{ asset('storage/' . $experience->bg_media_path) }}" controls style="width:100%; height:auto; display:block;"></video>
                                @else
                                    <img src="{{ asset('storage/' . $experience->bg_media_path) }}" alt="Current Background" style="width:100%; height:auto; display:block;">
                                @endif
                            </div>
                        @endif

                        <label class="pe-field-label" x-text="bg_media_type === 'video' ? 'Upload New MP4 File' : 'Upload New Background Image'"></label>
...
function notionEditorEdit() {
    return {
        ...
        activeBlockId: null,
        isSubmitting: false,
        lastSaved: '{{ $experience->updated_at ? $experience->updated_at->toIso8601String() : '' }}',
        slashMenuOpen: false,
        slashQuery: '',
        slashBlockId: null,
        slashActiveIndex: 0,
        ...
        loadContent() {
            let raw = @json(old('body_content', $experience->body_content));
        ...
        // ── Slideshow logic ──
        initSlideshow() {
            const existing = @json($experience->bg_gallery_images ?? []);
