    ...
    <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="bg-slate-900 border border-slate-850 p-8 rounded-2xl shadow-xl space-y-6">
        ...
        <div>
            ...
            <textarea name="description" 
                      ...
                      rows="6"
                      required
                      class="w-full bg-slate-950 border border-slate-800 focus:border-cyan-500/50 rounded-xl px-4 py-3 text-slate-200 text-sm outline-none focus:ring-1 focus:ring-cyan-500/20 transition-all duration-200 resize-none">{{ old('description', $project->description) }}</textarea>
            @error('description')
                <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 p-6 bg-slate-950/60 border border-slate-850 rounded-xl">
            <!-- Thumbnail File -->
            <div>
                ...
                @if($project->thumbnail_path)
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <span class="text-xs text-slate-500">Current active image</span>
                    </div>
                @endif
