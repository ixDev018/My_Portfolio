    ...
    <div class="pe-shell">
                    ...
                    <div class="pe-block"
                        ...
                        <template x-if="block.type === 'slideshow'">
                            <div style="width:100%;" class="pe-block-slideshow-wrap">
                                ...
                                {{-- Image Grid --}}
                                <div x-show="block.images && block.images.length > 0" style="display:flex; flex-wrap:wrap; gap:0.5rem; margin-bottom:0.5rem;">
                                    <template x-for="(img, idx) in block.images">
                                        <div style="position:relative; width:80px; height:80px; border-radius:0.4rem; overflow:hidden; border:1px solid #D8D4C8;">
                                            <img :src="img.src" style="width:100%; height:100%; object-fit:cover;">
                                            <button type="button" @click.stop="openCropper(block.id, idx)" title="Crop Image" style="position:absolute; bottom:4px; right:28px; background:rgba(0,0,0,0.6); color:#fff; border:none; border-radius:0.2rem; width:20px; height:20px; display:flex; align-items:center; justify-content:center; cursor:pointer;">
                                                <svg fill="currentColor" viewBox="0 0 24 24" style="width:12px;height:12px;"><path d="M17 15h2V7c0-1.1-.9-2-2-2H9v2h8v8zM7 17V1H5v4H1v2h4v10c0 1.1.9 2 2 2h10v4h2v-4h4v-2H7z"/></svg>
                                            </button>
                                            <button type="button" @click.stop="removeSlideshowImage(block.id, idx)" title="Remove Image" style="position:absolute; bottom:4px; right:4px; background:rgba(220,38,38,0.8); color:#fff; border:none; border-radius:0.2rem; width:20px; height:20px; display:flex; align-items:center; justify-content:center; cursor:pointer;">
                                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:12px;height:12px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                            </button>
                                        </div>
                                    </template>
                                </div>
        ...
        <div class="pe-sidebar" x-data="{ tab: 'media', featured: {{ old('featured') ? 'true' : 'false' }}, is_best_work: {{ old('is_best_work') ? 'true' : 'false' }}, is_top: {{ old('is_top') ? 'true' : 'false' }}, is_archived: {{ old('is_archived') ? 'true' : 'false' }} }">
            ...
            <div class="pe-sidebar-scroll">
                    ...
                    <div class="pe-field-row" style="margin-bottom: 1.25rem;">
                        <div class="pe-field" style="flex-direction:row; align-items:center; justify-content:space-between; background:#F7F5EE; padding:0.85rem 1rem; border-radius:0.5rem; border:1px solid #E2DDD3;">
                            ...
                            <label style="position:relative; display:inline-block; width:36px; height:20px;">
                                <input type="checkbox" name="show_story" value="1" {{ old('show_story', true) ? 'checked' : '' }} style="opacity:0; width:0; height:0;" onchange="this.nextElementSibling.style.background = this.checked ? '#6829AA' : '#C4BDB2'; this.nextElementSibling.firstElementChild.style.transform = this.checked ? 'translateX(16px)' : 'translateX(0)';">
                                <span style="position:absolute; cursor:pointer; top:0; left:0; right:0; bottom:0; background-color:{{ old('show_story', true) ? '#6829AA' : '#C4BDB2' }}; transition:.2s; border-radius:20px;">
                                    <span style="position:absolute; content:''; height:16px; width:16px; left:2px; bottom:2px; background-color:white; transition:.2s; border-radius:50%; transform:{{ old('show_story', true) ? 'translateX(16px)' : 'translateX(0)' }}"></span>
                                </span>
                            </label>
                        </div>
                    </div>
                ...
                <div class="pe-field-row">
                    <div class="pe-field">
                        ...
                        <select name="category" class="pe-field-select">
                            <option value="ui" {{ old('category') == 'ui' ? 'selected' : '' }}>UI/UX Design</option>
                            <option value="visual" {{ old('category') == 'visual' ? 'selected' : '' }}>Creative Visual</option>
                ...
                <div class="pe-field-row">
                    <div class="pe-field">
                        <label class="pe-field-label">Client</label>
                ...
                <div class="pe-field-row">
                    <div class="pe-field">
                        <label class="pe-field-label">Year</label>
                        <input type="text" name="year" class="pe-field-input"
                               value="{{ old('year') }}"
...
function comingSoonGallery() {
    return {
        images: @json(old('coming_soon_gallery', [])),
        ratio: '{{ old('coming_soon_gallery_ratio', '16:9') }}',
        isUploading: false,
        dragIndex: null,
